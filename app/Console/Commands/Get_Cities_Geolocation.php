...
class Get_Cities_Geolocation extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'cities:geolocation';
     ...
     * @var string
     */
    protected $description = 'Command has been created for feeling geo_lat nad geo_lng fields in cities table.';
    ...
    public function handle()
    {
        ...
        foreach ($cities as $c) {
            ...
            $addr .= $c->name_en;
            if ($c->state_id != null) {
                $addr .= ' ' . $c->state()->first()->name_en;
            }
            if ($c->country_id != null) {
                $addr .= ', ' . $c->country()->first()->name_en;
            }
            $geocode_url = "https://maps.googleapis.com/maps/api/geocode/json?address=" . urlencode($addr) . "&key=AIzaSyAr2PDr0fhwN5po1CjPpWbb6j0fJtDs4-4";
