...
class PaymentHelper{
  ...
  public $API_Endpoint, $PAYPAL_URL;
  
  public $USE_PROXY = false;
  public $version = "64";
  
  public function __construct($API_UserName, $API_Password, $API_Signature, $SandboxFlag)
  {
  ...
  /*   
	...
	*/
	public function CallShortcutExpressCheckout( $paymentAmount, $currencyCodeType, $paymentType, $returnURL, $cancelURL) 
	{
		...
		Session::put("currencyCodeType", $currencyCodeType);	  
	...
	*/
	public function GetShippingDetails( $token )
	{
	  ...
	  $ack = strtoupper($resArray["ACK"]);
		if($ack == "SUCCESS" || $ack=="SUCCESSWITHWARNING")
		{	
			Session::put('payer_id', $resArray['PAYERID']);
		} 
		return $resArray;
	}
	...
	'--------------------------------------------------------------------------------------------------------------------------------------------	
	*/
	public function ConfirmPayment( $FinalPaymentAmt,  $payerID = NULL)
	{
    /* Gather the information to make the final call to
		...
		$currencyCodeType = urlencode(Session::get('currencyCodeType'));
	...
	*/
    ...
    Session::put('nvpReqArray', $nvpReqArray);

		if (curl_errno($ch)) 
		{
			// moving to display page to display curl errors
			  Session::put('curl_error_no', curl_errno($ch)) ;
			  Session::put('curl_error_msg', curl_error($ch));
